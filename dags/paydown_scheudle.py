...
# TODO: Need to collect the appropriate data to work with from ❄️ look @ dim_loan and holidays and repayment_amount
...
def read_temp_csv_data(filepath):
...
def all_known_holidays(holiday_filepath):
...
def schedule_in_days(frequency):
    try:
        if frequency == "daily":
...
def calculate_all_paydown_schedules(filepath):
