def get_journal_entry_line(entry_type, account_internal_id, amount, memo):
    return f"""
    <ns1:line>
         <ns1:account xsi:type='platformCore:RecordRef' internalId='{account_internal_id}'/>
         <ns1:{entry_type}>{amount}</ns1:{entry_type}>
         <ns1:memo xsi:type='xsd:string'>{memo}</ns1:memo>
    </ns1:line>
    """
...
def get_manual_payment_payload(tran_date, rows, email, password, account, app_id):
    subsidiary = get_subsidiary(6)
    tran_date = get_tran_date(tran_date)
    lines = []
    for row in rows:
        if row['debit'] and int(row['debit']) != 0:
            lines.append(get_journal_entry_line('debit', row['internal_id'], abs(row['debit']), 'Manual Payment'))
        else:
            lines.append(get_journal_entry_line('credit', row['internal_id'], abs(row['credit']), 'Manual Payment'))
