...
def process_grouped_transactions(correlation_guid, df, client):
    # check subsidiary
    ...
    try:
            ...
            if values[0] is not None:
                line_list.append(
                    journal_entry_line(
                        ...
                        credit=values[0],
                        memo=f"platform_erp: {correlation_guid}",
                    )
                )
        ...
        if succeed and re.body.writeResponse.baseRef.internalId:
            ...
            raise ValueError(f"{status.statusDetail}")
...
with DAG(
    ...
    start_date=pendulum.datetime(
        2020, 5, 1, tzinfo=pendulum.timezone("America/Toronto")
    ),
) as dag:
                    ...
                    if len(group) == 2:
                        ...
                        try:
                            je_iid = process_grouped_transactions(
                                correlation_guid, group[1], client
                            )
                            logger.debug(f"Journal Entry uploaded: {je_iid}")
                            succeeded.append(("placeholder", je_iid, correlation_guid))
                        except ValueError as e:
                            logger.error(f"Journal Entry failed: {correlation_guid}")
                            logger.error(e)
