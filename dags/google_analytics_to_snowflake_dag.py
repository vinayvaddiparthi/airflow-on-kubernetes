...
with DAG("google_analytics_to_snowflake_dag", schedule_interval='@daily', default_args=default_args) as dag:
    ...
    def df_to_sql(schema: str, table: str, **context):
        ds = context['ds']
        prev_ds = datetime.strptime(ds, '%Y-%m-%d').date() - timedelta(days=1)
        ...
        while page_token is not None:
            ...
            with snowflake.connector.connect(
                ...
                engine = create_engine(f"snowflake://{snowflake_hook.host}.snowflakecomputing.com",
                                       poolclass=StaticPool, creator=lambda: conn)
                if response:
