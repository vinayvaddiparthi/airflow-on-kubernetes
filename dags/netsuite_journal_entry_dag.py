...
with DAG("new_journal_entry_dag", schedule_interval=None, default_args=default_args) as dag:
            ...
            for td in tds:
                ...
                mp_sql = f"""
                            select tli.general_ledger_account_number__c     as old_gl,
                                   ...
                                   sum(c2g__homevalue__c)                   as home_value,
                                   case
                                       when (
                                               (tli.general_ledger_account_number__c like '1008' and tli.debit__c > '0' and
                                                tli.c2g__linereference__c like '%EFT Collection%')
                                               or
                                               (tli.credit__c < '0' and tli.c2g__linedescription__c like '%TRXCAT01 - Collection%') and
                                               tli.general_ledger_account_number__c in ('2103', '1115', '1116')
                                           ) then 'Manual Payment cash collection'
                                       when (
                                               (tli.general_ledger_account_number__c like '1008' and tli.debit__c > '0' and
                                                tli.c2g__linereference__c like '%NSF Resubmit%')
                                               or
                                               (tli.credit__c < '0' and tli.c2g__linedescription__c like '%NSF Resubmit%') and
                                               tli.general_ledger_account_number__c in ('2103', '1115', '1116')
                                           ) then 'Manual Payment cash collection of NSF'
                                       else 'Error from Manual Payment' end as process
                            ...
                            where tli.createbyid = {cb} and created_date = '{cd}' and transaction_date = '{td_str}';
                """
