# This is a carbon copy of the zetatango_import.py dag but focuses the endpoint at staging instead of production
# Ideally this will be tinkered with to push data to staging when it needs to be played with and altered
# This will not push any data up to production and will be run once a week to avoid production as it is not as critical
import logging
import tempfile
...
def stage_table_in_snowflake(
    ...
    logging.info(f"start syncing table for staging: {table}")
    ...
    with snowflake_engine.begin() as tx, cast(
        ...
        with csv_filepath.open("w+b") as csv_filedesc:
            logging.info(f"copy {source_schema}.{table} for staging")
            ...
            if table in (
                ...
                result = tx.execute(
                    f"select coalesce(max(fields:updated_at::varchar), current_timestamp()) from {destination_schema}.{table}"  # nosec
                ).fetchone()
        ...
        try:
            logging.info(f"read {csv_filepath} for {table} for staging")
...
def create_dag() -> DAG:
    with DAG(
        dag_id="zetatango_import_to_staging",
        ...
        decryption_role = Variable.get("zetatango_staging_decryption_role")
        ...
        decrypt_idp_prod = RBACPythonOperator(
            task_id="zt-staging-elt-idp__pii_decryption",
            python_callable=decrypt_pii_columns,
            op_kwargs={
                "snowflake_connection": "snowflake_production",
...
if __name__ == "__main__":
    ...
    globals()["ztimportdagstaging"] = create_dag()
