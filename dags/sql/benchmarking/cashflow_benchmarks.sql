...
with monthly_balances as (
    ...
    group by merchant_guid, account_guid, month, batch_timestamp
    qualify rank() over (partition by merchant_guid, account_guid order by batch_timestamp desc) = 1
),
merchant as (
    select 
    guid,
...
merchant_industry_aggregated as (
    ...
    -- order by month desc, macro_industry, province
),
