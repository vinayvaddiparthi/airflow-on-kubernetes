    ...
    with DAG(
        ...
        for sobject in sobjects:
            dag << PythonOperator(
                ...
                retries=3,
                priority_weight=sobject.get("weight", 0),
                bash_command='export SNOWFLAKE_ACCOUNT="{{ dbt_hook.host }}" | export SNOWFLAKE_USERNAME="{{ dbt_hook.login }}" | export SNOWFLAKE_PASSWORD="{{ dbt_hook.password }}" | export SNOWFLAKE_DATABASE="ANALYTICS_PRODUCTION" | export SNOWFLAKE_ROLE="DBT_PRODUCTION" | export SNOWFLAKE_WAREHOUSE="ETL" | export ZETATANGO_ENV="PRODUCTION" | git clone https://{{gl_user}}:{{gl_token}}@gitlab.com/tc-data/curated-data-warehouse.git | cd curated-data-warehouse | dbt seed --profiles-dir . --target prod | dbt run --profiles-dir . --target prod | dbt snapshot --profiles-dir . --target prod',
            )
        return dag
