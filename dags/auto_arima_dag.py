...
def create_table(snowflake_connection: str) -> None:
    try:
        ...
        create_table_query = """CREATE TABLE IF NOT EXISTS "ZETATANGO"."CORE_PRODUCTION"."CASH_FLOW_PROJECTIONS"
        (MERCHANT_GUID VARCHAR NOT NULL, ACCOUNT_GUID VARCHAR NOT NULL, PROJECTIONS VARIANT NOT NULL)"""
    ...
    try:
        ...
        with snowflake_engine.begin() as tx, tempfile.TemporaryDirectory() as path:
            ...
            statements = [
                ...
                (
                    f'MERGE INTO "ZETATANGO"."CORE_PRODUCTION"."CASH_FLOW_PROJECTIONS" cash_flow_projections'
                    f" USING (SELECT '{merchant_guid}' AS merchant_guid, '{account_guid}' AS account_guid,"
                    ...
                    f" cash_flow_projections.account_guid = projection.account_guid"
                    f" WHEN MATCHED THEN UPDATE SET cash_flow_projections.projections = projection.projection"
                    f" WHEN NOT MATCHED THEN INSERT (merchant_guid, account_guid, projections)"
                    f" VALUES (projection.merchant_guid, projection.account_guid, projection.projection)"
                ),
            ]
        ...
        cash_flow_query = """
            WITH object_debits_credits AS (
              SELECT account_guid, merchant_guid, date,
                     OBJECT_CONSTRUCT('debits', debits, 'credits', credits, 'balance', opening_balance) AS object
