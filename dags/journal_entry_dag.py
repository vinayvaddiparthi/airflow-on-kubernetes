...
with DAG("journal_entry", schedule_interval='0 13 * * *', default_args=default_args) as dag:
    snowflake_hook = BaseHook.get_connection("snowflake_erp")
    netsuite_hook = BaseHook.get_connection("netsuite")
    netsuite = {"email": netsuite_hook.login,
    ...
    def get_tli_by_created_date(**kwargs):
        ...
        with open('airflow/dags/netsuite_extras/queries/get_tli_by_created_date.sql') as f:
            sql_template = Template(f.read())
        sql = sql_template.render(created_date=cd, env=f'{env}', create_date_trim=date_tag)
    ...
    def get_unbalanced_tli_by_subsidiary(**kwargs):
        ...
        with open('airflow/dags/netsuite_extras/queries/get_unbalanced_tran_by_subsidiary.sql') as f:
            sql_template = Template(f.read())
        ...
        with snowflake.connector.connect(
            ...
            cur.execute(sql)
            for role in roles:
                cur.execute(f"grant select on erp.{env}.tli_unbalanced_{date_tag} to role {role};")
    ...
    def update_tli_unbalanced_historical(**kwargs):
        ...
        with open('airflow/dags/netsuite_extras/queries/update_tli_unbalanced_historical.sql') as f:
            sql_template = Template(f.read())
            sql = sql_template.render(execition_time=execution_time, created_date_trim=date_tag, env=env)
        with snowflake.connector.connect(
                user=snowflake_hook.login,
                password=snowflake_hook.password,
                account=snowflake_hook.host,
    ...
    def send_journal_entries(**kwargs):
        ...
        with open('airflow/dags/netsuite_extras/queries/create_journal_entry_line.sql') as f:
            sql_template = Template(f.read())
            sql = sql_template.render(created_date=created_date, env='test',
    ...
    def upload_journal_entry(created_date, rows):
        ...
        try:
            ...
            payload = netsuite_helper.get_journal_entry_payload(created_date,
                                                                rows,
                                                                netsuite['email'],
                                                                netsuite['password'],
                                                                netsuite['account'],
            ...
            r = requests.post(netsuite['endpoint'],
                              headers=headers,
                              data=payload)
            ...
            if write_response["platformCore:status"]["@isSuccess"] == "true":
                ...
                log_status(je_internal_id, 'TRUE', '', execution_time, created_date, tran_date, subsidiary_id)
            else:
                code = write_response["platformCore:status"]['platformCore:statusDetail']['platformCore:code']
                message = write_response["platformCore:status"]['platformCore:statusDetail']['platformCore:message']
                error_message = code + ":" + message
                log_status('', 'FALSE', error_message, execution_time, created_date, tran_date, subsidiary_id)
    ...
    def log_uploaded(**kwargs):
        ...
        with open('airflow/dags/netsuite_extras/queries/log_uploaded.sql') as f:
            sql_template = Template(f.read())
        log_history_sql = sql_template.render(env='test', created_date_trim=created_date.replace('-', ''))
        with snowflake.connector.connect(
                user=snowflake_hook.login,
                password=snowflake_hook.password,
                account=snowflake_hook.host,
    ...
    def log_status(je_internal_id, uploaded, error_msg, execution_time, created_date, transaction_date, subsidiary_id):
        print(f"log_status:{uploaded}")
        with open('airflow/dags/netsuite_extras/queries/log_status.sql') as f:
            sql_template = Template(f.read())
        log_status_sql = sql_template.render(je_internal_id=je_internal_id,
    ...
    task_get_unbalanced_tli_by_subsidiary >> [task_send_email_for_unbalanced_tli, task_create_journal_entry]
